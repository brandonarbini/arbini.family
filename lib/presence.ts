 ...
 *
 * A stay is the only thing that puts somebody anywhere. There is no fallback to where a person
 * usually is, because the board would then be answering "where is everyone" with a fact nobody
 * stated today, and a gathering countdown built on that is wrong in exactly the case anybody
 * would act on. A day nothing covers resolves to `null` — unknown, and rendered as such.
 *
